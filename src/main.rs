...
fn write_relo_info(instruction_stream: &mut Vec<u8>, location: usize, value: i32) {
    for (idx, y) in value.to_le_bytes().iter().enumerate() {
        assert!(instruction_stream[location + idx] == 0x00);
...
fn interpret_bf(bf: &str) {
    ...
    let mut epilogue = vec![
        0x48, 0x81, 0xC4, 0x38, 0x75, 0x00, 0x00, 0x41, 0x5E, 0x41, 0x5D, 0x41, 0x5C, 0x5D, 0xC3,
    ];
    ...
    for instr in bf.chars() {
        match instr {
            ...
            '.' => {
                let mut call_putchar = vec![0x41, 0x0F, 0xB6, 0x3C, 0x24, 0x41, 0xFF, 0xD5];
                instruction_stream.append(&mut call_putchar);
            }
            ',' => {
                let mut call_getchar = vec![0x41, 0xFF, 0xD6, 0x41, 0x88, 0x04, 0x24];
                instruction_stream.append(&mut call_getchar);
            }
            '[' => {
                let mut make_label = vec![
                    0x41, 0x80, 0x3C, 0x24, 0x00, 0x0F, 0x84, 0x00, 0x00, 0x00, 0x00,
                ];
                instruction_stream.append(&mut make_label);
                relro_list.push(instruction_stream.len());
            }
            ']' => {
                let mut fix_label = vec![
                    0x41, 0x80, 0x3C, 0x24, 0x00, 0x0F, 0x85, 0x00, 0x00, 0x00, 0x00,
                ];
    ...
    type Jitfunc = fn(*const libc::c_void, *const libc::c_void, *const libc::c_void);
    ...
    unsafe {
        ...
        libc::memcpy(
            mem,
            instruction_stream.as_ptr() as *const libc::c_void,
            instruction_stream.len(),
        );
        let f = std::mem::transmute::<
            *mut libc::c_void,
            fn(*const libc::c_void, *const libc::c_void, *const libc::c_void),
        >(mem);
        f(
            libc::memset as *const libc::c_void,
            libc::putchar as *const libc::c_void,
            libc::getchar as *const libc::c_void,
        );
    }
